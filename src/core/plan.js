import { assertValidPlanDocument, PLAN_SCHEMA_VERSION } from "../contracts/plan_contract.js?v=20260917-player-partners-v1";
...
import { createInitialExperienceState } from "../rulesets/vw2r_experience.js?v=20260917-player-partners-v1";
...
import { ABILITY_FORM_STATE_VERSION, applyCombatantFormState, desiredWeatherAbilityForm } from "../rulesets/form_rules.js?v=20260917-player-partners-v1";
...
export function createPlanDocument({
  ...
  if (playerPartner) {
    ...
    if (format !== 'doubles' || !binding?.partnerOptions.some(option => option.trainerId === playerPartner.trainerId)) throw new Error('Invalid player partner selection');
    ...
    if (!owned.length || !allied.length || allied.some(mon => mon.source.partyOwnerId !== playerPartner.trainerId)) throw new Error('Invalid player partner roster');
