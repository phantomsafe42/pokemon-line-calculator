...
async function initialize(datasetBaseUrl, trainerAiBaseUrl, gameId, role) {
  ...
  const datasetModule = await import("../adapters/standardized_dataset.js?v=20260914-public-load-performance-v1");
  ...
  if (role === "resolver") {
    ...
    const trainerAiModule = await import("../adapters/trainer_ai.js?v=20260914-public-load-performance-v1");
...
self.addEventListener("message", async event => {
  ...
  try {
    if (type === "initialize") {
      const result = await initialize(payload.datasetBaseUrl, payload.trainerAiBaseUrl, payload.gameId, payload.role);
