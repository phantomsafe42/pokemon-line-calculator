...
async function initialize(datasetBaseUrl, datasetHostedPrefix, trainerAiBaseUrl, trainerAiHostedPrefix, trainerAiLazyResources, hostedRelease, gameId, role) {
  ...
  const datasetModule = await import("../adapters/standardized_dataset.js?v=20260917-multi-corrections-v2");
  ...
  if (role === "resolver") {
    const [plannerModule, combatantMovesModule] = await Promise.all([
      import("../core/planner.js?v=20260917-multi-corrections-v2"),
    ...
    const trainerAiModule = await import("../adapters/trainer_ai.js?v=20260917-multi-corrections-v2");
