(function (root, factory) {
    ...
    function calculateWithResolvedDownload(runtime, attacker, defender, move, field, downloadAlreadyResolved) {
        // The pinned calculator normally applies Download on every calculation.
        // Stateful callers have already resolved entry effects against the actual
        // opposing field. Download has no remaining damage-time effect.
        const suppressed = downloadAlreadyResolved === true
            ? [attacker, defender].map(mon => mon.ability === "Download")
            : [false, false];
        const pokemon = [attacker, defender];
        try {
            pokemon.forEach((mon, index) => { if (suppressed[index]) mon.ability = ""; });
            const result = runtime.calc.calculate(runtime.generation, attacker, defender, move, field);
            [result.attacker, result.defender].forEach((mon, index) => {
                if (suppressed[index]) mon.ability = "Download";
            });
            return result;
        } finally {
            pokemon.forEach((mon, index) => { if (suppressed[index]) mon.ability = "Download"; });
        }
    }
        ...
        try {
            ...
            const hitResults = hitOptions.map(hitOption => {
                if (simulation) {
                    ...
                    const result = calculateWithResolvedDownload(runtime, attackerPokemon, defenderPokemon, move, field, request.downloadAlreadyResolved);
