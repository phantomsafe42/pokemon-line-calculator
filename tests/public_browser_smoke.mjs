...
try {
  ...
  const state = await evaluate(page, `(async () => {
    ...
    check(partySelect.value === '' && partySelect.selectedOptions[0].textContent === 'New Party', 'default manual New Party');
    ...
    const savedPartyId = partySelect.options[1].value;
    ...
    check(partySelect.value === '' && grid.children[6].classList.contains('is-selected'), 'edit manual selection preserves chosen member');
    change(partySelect, savedPartyId);
    dialog.close();
    document.getElementById('new-plan').click();
    check(partySelect.value === '' && !grid.querySelector('.is-selected') && saveParty.disabled, 'new line resets saved Party to New Party');
  ...
  await evaluate(page, `(() => {
    ...
    change('context-party-select', document.getElementById('context-party-select').options[1].value);
