...
- One active IndexedDB draft with crash/refresh recovery, export freshness, destructive-transition confirmation, selected-node output, current-fingerprint reference validation, and atomic import-time migration. An older exported line is replayed under the current mechanics before it becomes the active draft, so a successful import is immediately editable; a failed migration leaves the imported document uninstalled rather than opening it read-only.
...
The Notes column includes a generated, read-only Trainer AI forecast above the user's per-node editable notes. It executes the Dataset-owned evaluator profiles in the resolver Worker and presents broad player-facing likelihoods from **Very Unlikely** through **Guaranteed**, with equally likely options called out explicitly. **Turn actions** combine moves, trainer items, and voluntary switches through their real phase precedence; **Replacement after a faint** separately forecasts which reserve the Trainer AI will send out. Move explanations expose the pre-selection incentive ledger: every legal move-target candidate starts at 100, documented AI rules add or remove points, and only after those scores are finalized does the highest final score enter the source-defined tie selector. A rule percentage is its source scoring chance in the supplied state; PLC never replaces it with a probability conditioned on that move eventually winning. Items and switches retain their actual eligibility, gate, priority, or ranking rules rather than being given invented move scores.

The supported VW2R, Renegade Platinum, PK Nightmare Update Final, and SSSG PChal builds bind canonical PLC moves to exact numeric identities and answer state-backed move, battler, type, status, stage, weather and action queries. Gen 4 AI damage ranking executes the Dataset's source-translated AI damage program, including its differences from actual battle damage; Gen 5 uses the shared calculator under its declared AI rules. In multi-active battles, PLC evaluates actors in the Dataset-declared order, carrying RNG state, consumed trainer-item slots and already-selected switch reservations into each later actor. Exact evaluator fractions remain available internally for validation. When the only missing input is the hidden Gen 4 LCRNG seed, the educational forecast runs a deterministic spread of exact stateful evaluations and converts the resulting weight into a qualitative band; it does not claim to recover the live seed or present a fake precise percentage. Missing commands, action semantics, or required battle state appear as a **Forecast error**, not as ordinary uncertainty.

Forecast visibility follows the generated Dataset binding's `consumerActivation.enabled`, not a PLC game-name whitelist. The Storm Silver selector currently binds only the hash-pinned supported Sacred Gold PChal build, not arbitrary SG/SS releases. PK is enabled for Nightmare Update Final after the approved eight-mask/four-nature authority resolution; its 484 supported trainer bindings exclude five workbook-only references. The older installed PK ROM is not interchangeable with Final and was not replaced. HGSS trainer items stop at the first chosen bag item and use a different remaining-party threshold from Platinum; those policies are explicit Dataset fields.
...
The local header includes a persistent **Local View / Public View** switch. Public View hides Output State, Battle Tracker, Live Edit, and local status messaging while continuing to run the same current local source and browser state; switching back restores those controls without reloading or discarding work. The generated public artifact remains permanently public-only and does not include the switch or private integration markup.
...
# Local test build: Drafts, Free Calc, and paired encounters

- The PLC, Boxes, and Drafts tabs are game-scoped. Save Draft retains the complete line and incomplete action selections in IndexedDB; Export Line downloads the selected turns. Clearing browser site data removes local drafts and Boxes, so export anything important.
- Box parties and setup parties share editable cards with drag handles and left/right keyboard reordering. Setup item and major-status edits update the shared Box record; HP remains a plan-specific starting condition.
- Free Calc clones the selected decision state. Close restores the original, Add retains a same-turn manual branch, and Save as Draft creates an independent Turn 1 line plus a new Box/Party. Manual states never request AI Forecast. Free Calc does not apply switch/entry effects or modify existing Box records.
- Free Calc uses plan schema v5. Ordinary plans retain their existing schema. Live Edit is disabled for v5 plans until Overlay supports manual-state presentation; no temporary manual edits are sent to it.
- Explicit Dataset encounter groups drive partnered Doubles. Enemy reserves remain owned by their trainer and fixed slot. VW2R includes 15 documented pre-League pairs: ten Double-only and five with a Singles/default or paired-Doubles choice. The five field-pair trigger classifications use retail encounter documentation cross-checked against the pinned Redux pages, not a final-ROM script disassembly.
- This is a local, unaccepted build, not a public release. Output State, Battle Tracker, and local Live Edit functionality remain present.
