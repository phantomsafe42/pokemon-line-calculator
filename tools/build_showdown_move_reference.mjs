...
const overridesByGame = Object.fromEntries(datasetProfiles.map(profile => {
  const generationRecords = recordsByGeneration[profile.generation] || {};
  const overrides = Object.fromEntries(Object.entries(recordsByGame[profile.gameId] || {}).filter(([moveId, record]) => (
    JSON.stringify(record) !== JSON.stringify(generationRecords[moveId])
  )));
  return [profile.gameId, overrides];
}));
const moveIdsByGame = Object.fromEntries(Object.entries(recordsByGame).map(([gameId, records]) => [gameId, Object.keys(records).sort()]));
for (const profile of datasetProfiles) {
  for (const [moveId, record] of Object.entries(recordsByGame[profile.gameId] || {})) {
    const reconstructed = overridesByGame[profile.gameId]?.[moveId] || recordsByGeneration[profile.generation]?.[moveId];
    if (JSON.stringify(reconstructed) !== JSON.stringify(record)) {
      throw new Error(`Compressed Showdown reference changed ${profile.gameId}:${moveId}`);
    }
  }
}

const header = `// Generated by tools/build_showdown_move_reference.mjs. Do not edit by hand.\n`;
const body = `${header}export const SHOWDOWN_MOVE_REFERENCE_BY_GAME = Object.freeze(${JSON.stringify(overridesByGame, null, 2)});\n\nexport const SHOWDOWN_MOVE_REFERENCE_BY_GENERATION = Object.freeze(${JSON.stringify(recordsByGeneration, null, 2)});\n\nexport const SHOWDOWN_MOVE_REFERENCE_IDS_BY_GAME = Object.freeze(${JSON.stringify(moveIdsByGame, null, 2)});\n\nexport const SHOWDOWN_MOVE_REFERENCE = Object.freeze(Object.fromEntries((SHOWDOWN_MOVE_REFERENCE_IDS_BY_GAME["volt-white-2r"] || []).map(moveId => [moveId, SHOWDOWN_MOVE_REFERENCE_BY_GAME["volt-white-2r"]?.[moveId] || SHOWDOWN_MOVE_REFERENCE_BY_GENERATION[5]?.[moveId]])));\n\nexport const SHOWDOWN_REFERENCE_SOURCE = Object.freeze(${JSON.stringify({
  ...
  policy: "Each standardized Dataset move resolves through its explicit mechanicsBase into the pinned Showdown historical mod stack at the game's declared damage generation. Generation records provide a shared base and game-scoped records contain only exact overrides, preserving divergent ROM-hack mechanics without duplicating identical definitions. Later backported moves retain their canonical structured definition while the selected Dataset remains authoritative for displayed move facts."
}, null, 2)});\n`;
