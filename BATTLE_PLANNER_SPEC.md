...
- Move explanations expose the pre-selection incentive-point ledger. Each legal move-target candidate begins at the profile's declared initial score—100 for supported Generation 4 and 5 profiles. Every reached executable score adjustment shows the documented points added or removed, its source random-gate probability, and the educational reason. Those probabilities are captured before maximum-score comparison and tie selection; conditioning them on the move later winning is forbidden. Final score outcomes use those pre-selection source marginals and must never be weighted by the hidden-seed action ensemble. Source-defined tie selection remains distinct. Trainer items, voluntary switches, and replacements use their real eligibility, gate, priority, or ranking systems; PLC must not invent a 100-point ledger for phases that do not use one.
- The supported VW2R, Renegade Platinum, PK Nightmare Update Final and SSSG PChal builds run their Generation 5 and Generation 4 evaluators in the resolver Worker with exact numeric move/item bindings and state-backed command/action queries. Gen 4 uses the source-translated Dataset AI damage program; Gen 5 uses the shared calculator under its declared AI rules. If the exact evaluator succeeds, the forecast derives directly from it. When a supported Gen 4 path is blocked only by the player's inability to observe the exact unsigned 32-bit `state.random.g4LcrngSeed`, the shared forecast layer runs a deterministic bounded ensemble of exact stateful seed executions and maps the result to a likelihood band. This is modeled player guidance, not an exact probability claim or recovered emulator state.
- Dataset's generated `consumerActivation.enabled` owns panel availability. A new game is not activated by recognizing its generation or sharing another game's program names. SSSG's first-usable-item scan and remaining-party threshold are explicit profile fields; Platinum's continued scan and stale-result bug remain separate. PK's 484 Nightmare Final bindings are enabled after the scoped eight-mask/four-nature authority approval; old workbook values remain in the Dataset ledger, five unverified reference records stay excluded, and the older installed ROM is not treated as Final.
...
# September 5 local implementation notes — not yet accepted

Saved lines use a separate `plc-saved-lines` IndexedDB database, retaining game identity, complete plan, selected decision/review node, incomplete action drafts and selected preview outcome. Draft identity includes plan identity and creation time; separate clean starts do not overwrite one another.

Free Calc uses explicit schema-v5 same-turn `manualTransitions`. The manual state is a cloned baseline, not a resolved switch; edits remain isolated until Add or Save as Draft. Recalculation replays explicit manual snapshots and then resolves descendant actions. No AI Forecast or Overlay Live Edit is attempted for these manual plans. The existing ordinary schemas and public/private feature boundaries are preserved.

Partnered enemy Doubles consume Dataset groups, with `game.partyOwnership.enemy.slotOwnerIds` and per-combatant source owner/local-party-slot identity. Switch, pivot, phazing and replacement eligibility respect that partition; an exhausted owner leaves an empty slot. AI party queries, trainer flags/bags and replacement mappings are owner-specific. Normal single-trainer Doubles retain their shared reserve pool. Optional Singles-vs-Doubles is driven only by an explicit format-choice field, not encounter-mandatory flags.
